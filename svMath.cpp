...
Real3 Point::tangent() const
{
  ...
  Real3 result = normalize(cross(binormal, normal));
  return result;
}
...
Real3 Point::binormal() const
{
  Real3 t = tangent();
  Real3 normal = position;
  Real3 result = normalize(cross(normal, t));
  return result;
}
...
{
  Real3 bcCross = cross(planeB.normal(), planeC.normal());
  Real denom = dot(planeA.normal(), bcCross);
...
std::vector<Real3> splitSphericalLineSegment(const Point& start, const Point& end, Real deltaAngle)
{
  ...
  for (float angle = deltaAngle; angle<distance; angle+=deltaAngle)
  {
    ...
    Mat4 rotation = Mat4::rotation(Rad(angle), direction);
    //Real3 pos = normalize(Real3(rotation * Real4(start.position, 1.0)));
...
Real computeTriangleArea(const Real3& p0, const Real3& p1, const Real3& p2)
{
  Real3 v12 = p2 - p1;
  Real3 v02 = p2 - p0;
  Real3 v12n = normalize(v12);
  Real t = dot(v02, v12n);
  Real3 c = p2 - v12n * t;
  Real d = distance(p0, c);
  Real l12 = length(v12);
  return l12 * d * 0.5;
}
