...
inline void save(Archive& archive, sv::Real3 const& v)
{
    archive(v.x(), v.y(), v.z());
}

template <class Archive>
inline void load(Archive& archive, sv::Real3& v)
{
    archive(v.x(), v.y(), v.z());
}

template <class Archive>
inline void save(Archive& archive, sv::Real2 const& v)
{
    archive(v.x(), v.y());
}

template <class Archive>
inline void load(Archive& archive, sv::Real2& v)
{
    archive(v.x(), v.y());
}
...
inline Plane Plane::transform(const Mat4 transform) const {
  ...
  res.m_distance = m_distance - dot(getTransformPosition(Mat4(transform)), res.m_normal);
    ...
    : m_face(face), m_height(p)
{
  ...
  {
    case CF_POSX:
      m_surfacePoint = Real3(1.0, t, -s);
      break;
    case CF_NEGX:
      m_surfacePoint = Real3(-1.0, t, s);
      break;
    case CF_POSY:
      m_surfacePoint = Real3(t, 1.0, s);
      break;
    case CF_NEGY:
      m_surfacePoint = Real3(-t, -1.0, s);
      break;
    case CF_POSZ:
      m_surfacePoint = Real3(s, t, 1.0);
      break;
    case CF_NEGZ:
      m_surfacePoint = Real3(-s, t, -1.0);
      break;
...
template <typename T>
inline PositionT<T>::PositionT(ECubeFace face, const typename PositionT<T>::Real3& stp)
    : Position(face, stp.s, stp.t, stp.p)
{
}

template <typename T>
inline typename PositionT<T>::Real3 PositionT<T>::stpCoords() const
{
  switch (m_face)
  {
    case CF_POSX:
      return Real3(-m_surfacePoint.z(), m_surfacePoint.y(), m_height);
    case CF_NEGX:
      return Real3(m_surfacePoint.z(), m_surfacePoint.y(), m_height);
    case CF_POSY:
      return Real3(m_surfacePoint.z(), m_surfacePoint.x(), m_height);
    case CF_NEGY:
      return Real3(m_surfacePoint.z(), -m_surfacePoint.x(), m_height);
    case CF_POSZ:
      return Real3(m_surfacePoint.x(), m_surfacePoint.y(), m_height);
    case CF_NEGZ:
      return Real3(-m_surfacePoint.x(), m_surfacePoint.y(), m_height);
    default:
      assert(false);
